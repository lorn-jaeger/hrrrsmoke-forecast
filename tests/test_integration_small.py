...
def test_small_pipeline_ingest_and_station_daily(monkeypatch, tmp_path: Path):
...
split:
  ...
  test_end: "2021-01-05"
    ...
    monkeypatch.setattr(station_daily, "_resolve_station_timezones", lambda stations: pd.Series(["UTC"] * len(stations), index=stations.index))

    station_daily.run(cfg)

    joined = pd.read_parquet(cfg.paths.station_daily_output)
    assert not joined.empty
    assert "massden_daily_mean" in joined.columns
    assert "colmd_daily_mean" in joined.columns
