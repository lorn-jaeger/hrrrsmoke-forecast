...
from gribcheck.pipelines import accuracy, pm_ingest, station_daily
...
def test_accuracy_can_run_from_partial_checkpoint(monkeypatch, tmp_path: Path):
...
split:
  ...
  test_end: "2021-01-05"
    ...
    monkeypatch.setattr(accuracy, "load_filtered_fire_records", lambda _cfg: [])

    stats = accuracy.run(cfg, workers=1, from_checkpoint=True)
